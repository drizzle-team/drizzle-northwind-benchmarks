...
bench("Knex ORM customer: search", async () => {
  for (const companyName of searches) { await db("customer").whereRaw("company_name LIKE ?", [`%${companyName}%`]); }
});
...
bench("Knex ORM Products: search", async () => {
  for (const name of searches) { await db("product").whereRaw("name LIKE ?", [`%${name}%`]); }
});
